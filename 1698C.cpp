...
int main(void){
	...
	while(++tcase, t--){
		...
		sort(all(a));
		if(pos > 2 || neg > 2) msg(NO);
		else{
			vi res;
			for(int i = 0; i < n; ++i) if(a[i] != 0) res.pb(a[i]);
			if(f) res.pb(0);
			sort(all(res));
			bool ok = 1;
			for(int i = 0; i < sz(res); ++i){
				for(int j = i + 1; j < sz(res); ++j){
					for(int k = j + 1; k < sz(res); ++k){
						int sum = res[i] + res[j] + res[k];
						if(bin_sc(res, sum) == 0) ok = 0;
					}
				}
			}
			cout << (ok?"YES":"NO") << ln;
