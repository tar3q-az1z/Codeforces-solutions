...
int main(void){
    ...
    while(++tcase, t--){
        ...
        for(int i = 0; i < n; ++i) {
            int rest = k;
            if(rest == 1) {
                if(i == 0) ans = max(ans, a[i] + a[n - 1]);
                else if(i == n - 1) ans = max(ans, a[i] + a[0]);
                else ans = max(ans, a[i] + max(a[0], a[n - 1]));
            }else {
                vii tm;
                for(int x = 0; x < n; ++x) if(x != i) tm.pb(a[x]);
                sort(begin(tm), end(tm));
                reverse(begin(tm), end(tm));
                i64 tans = 0;
                for(int x = 0; x < k; ++x) tans += tm[x];
