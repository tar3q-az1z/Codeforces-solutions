...
int main(void){
	...
	while(++tcase, t--){
		...
		suff[n - 1] = a[n - 1];
		lli ans = -1;
		for(lli i = 1; i < n; ++i) pref[i] += pref[i - 1] + a[i];
		for(lli i = n - 2; i >= 0; --i) suff[i] += suff[i + 1] + a[i];
		// printv(a);
		// printv(pref);
		// printv(suff);
		for(lli i = 1; i <= k; ++i){
			lli f = 2 * i;
			// ans = max(pref[n - 1] - pref[f - 1], suff[0] - suff[n - i]);
			lli j = k - i;
			if(n - j == n){
				ans = max(ans, pref[n - 1] - pref[f - 1]);
				break;
			}
			ans = max(ans, llabs((pref[n - 1] - pref[f - 1]) - suff[n - j]));
			
		}
		ans = max(ans, suff[0] - suff[n - k]);
		cout << ans << ln;
	}
	return 0;
}
