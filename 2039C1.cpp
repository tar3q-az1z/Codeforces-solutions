...
int main(void){
    ...
    while(++tcase, t--){
        ...
        cin >> x >> m;
        i64 cnt = 0;
        // for d to be divisor of x, d <= x / 2
        // the anser exists only when msb of both x, y is set bit
        // otherwise not, cause x ^ y will always lead to equal msb of x or y and > x or > y

        for(i64 y = binpow(2LL, 1LL * log2(x)); y < 2 * x && y <= m; ++y) {
            i64 exor = x ^ y;
            if(x != y && (x % exor == 0 || y % exor == 0)) ++cnt;
        }
        cout << cnt << ln;
