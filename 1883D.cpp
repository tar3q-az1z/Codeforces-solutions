...
int main(void){
	...
	while(++tcase, t--){
		...
		int q;
		cin >> q;
		pbds<int, null_type, less_equal<int>> left, right;
		while(q--){
			int a, b;
			char ch;
			cin >> ch >> a >> b; 
			if(ch == '+'){
				...
				left.erase(left.find_by_order(left.order_of_key(a)));
				right.erase(right.find_by_order(right.order_of_key(b)));
			}
			if(sz(left) == 0){
				cout << "NO" << ln;
				continue;
			}
			if(*right.find_by_order(0) < *left.find_by_order(sz(left) - 1)) cout << "YES" << ln;
			else cout << "NO" << ln;
		}
	}
	return 0;
