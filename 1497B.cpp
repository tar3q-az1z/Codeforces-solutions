...
int main(void){
	...
	while(++tcase, t--){
		...
		/*
			if (a + b) % m = 0, then
			((a % m) + (b % m)) % m = 0 
			=> (r1 + r2) % m = 0, where r1 = a % m, r2 = b % m
			so, r1 + r2 = m
		*/
		int ans = (rem[0] == 0? 0 : 1);
		for(int i = 1; i < m; ++i){
			int curr1 = min(rem[i], rem[m - i]), curr2 = max(rem[i], rem[m - i]);
			if(curr2 == 0) ans += 0;
			else if(curr1 == 0) ans += curr2;
			else if(curr1 == curr2) ans += 1;
			else{
				ans += 1 + curr2 - (2 + curr1 - 1);
			}
			rem[i] = rem[m - i] = 0;
		}
		cout << ans << '\n';
