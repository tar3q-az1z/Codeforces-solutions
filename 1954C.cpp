...
int main(void){
	...
	while(++tcase, t--){
		...
		for(int i = min(idA, idB); i < sz(a); ++i){
			(idA == inf.i and b[i] > a[i]? swap(a[i], b[i]): (idB == inf.i and a[i] > b[i]? swap(a[i], b[i]) : swap(a[i], a[i])));
		}
		cout << a << ln << b << ln;
	}
	return 0;
