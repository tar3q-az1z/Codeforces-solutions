...
int main(void){
	...
	while(++tcase, t--){
		...
		//printm(ans);
		int onec = ans[1], twoc = ans[2];
		// dbug(onec, twoc);
		if(onec + twoc == n){
			if(onec == 2 && twoc == n - 2) msg(bus topology);
			else msg(ring topology);
		}else{
			if(onec == n - 1 && ans[n - 1] == 1) msg(star topology);
			else msg(unknown topology);
		}
	}
	return 0;
}
