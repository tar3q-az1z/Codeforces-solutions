...
int main(void){
	...
	while(++tcase, t--){
		...
		int ok = 0;
		for(int i = 0; i < n; ++i){
			if(b[n] >= min(a[i], b[i]) && b[n] <= max(a[i], b[i])) ok = 1;
		}
		if(ok){
			for(int i = 0; i < n; ++i) cnt += llabs(b[i] - a[i]);
			++cnt;
		}else{
			lli mn = inf.i;
			for(int i = 0; i < n; ++i) cnt += llabs(b[i] - a[i]);
			for(int i = 0; i < n; ++i) mn = min(mn, min(llabs(b[n] - a[i]), llabs(b[n] - b[i]))); 
			cnt += mn;
			++cnt;
		}
		cout << cnt << ln;
