...
int main(void){
	...
	while(++tcase, t--){
		...
		for(int i = 0; i < m - 1; ++i){
			...
			if(mm[a[i + 1]] > (mm[a[i]] + d)){
				ans = 0;
				break;
			}
			ans = min(ans, mm[a[i + 1]] - mm[a[i]]);
			if(mm[a[i + 1]] - mm[a[i]] <= d){
				int needed = d - (mm[a[i + 1]] - mm[a[i]]) + 1;
				int have = 0;
				if(mm[a[i]] > 1) have += mm[a[i]] - 1;
				if(mm[a[i + 1]] < n) have += (n - mm[a[i + 1]]);
				if(needed <= have) ans = min(ans, needed);
			}
		}
		cout << ans << ln;
	}
