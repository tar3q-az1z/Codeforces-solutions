...
int main(void){
	...
	while(t--){
		...
		for(int i = 0; i < sz(s); ++i){
			if(s[i] == '1') ones.pb(i);
			else if(s[i] == '2') twos.pb(i);
			else threes.pb(i);
		}
		if(sz(ones) == 0 || sz(twos) == 0 || sz(threes) == 0) feed(0);
		else{
			int mn = inf.i;
			for(int i = 0; i < sz(s); ++i){
				array<int, 3> l = {inf.i, inf.i, inf.i};
				if(s[i] == '1'){
					l[0] = i;
					if(lbd(twos, i) != twos.end()) l[1] = *lbd(twos, i);
					if(lbd(threes, i) != threes.end()) l[2] = *lbd(threes, i);
					sort(all(l));
					mn = min(mn, l[2] - l[0] + 1);
				}else if(s[i] == '2'){
					l[0] = i;
					if(lbd(ones, i) != ones.end()) l[1] = *lbd(ones, i);
					if(lbd(threes, i) != threes.end()) l[2] = *lbd(threes, i);
					sort(all(l));
					mn = min(mn, l[2] - l[0] + 1);
				}else{
					l[0] = i;
					if(lbd(ones, i) != ones.end()) l[1] = *lbd(ones, i);
					if(lbd(twos, i) != twos.end()) l[2] = *lbd(twos, i);
					sort(all(l));
					mn = min(mn, l[2] - l[0] + 1);
				}
			}
