...
int main(void){
    ...
    while(++tcase, t--){
        i64 n, m, L;
        cin >> n >> m >> L;
        vector<pair<i64, i64>> x(n + m);
        for(i64 i = 0; i < n; ++i) cin >> x[i].first >> x[i].second;
        for(i64 i = n; i < n + m; ++i) {
            cin >> x[i].first >> x[i].second;
            x[i].second *= -1;
        }
        sort(begin(x), end(x));
        priority_queue<i64> pows;
        i64 tm = 1, flag = 0;
        for(auto &[k, v]: x) {
            if(v < 0) pows.push(-1 * v);
            else {
                i64 now;
                if(!pows.empty()) now = pows.top();
                else now = 0;
                ++v;
                v -= tm;
                while(v >= k && !pows.empty()) {
                    tm += now;
                    v -= now;
                    pows.pop();
                    now = pows.top();
