...
// for pair & pair of .......
template <typename T, typename S> 
ostream& operator<<(ostream& os, const pair<T, S>& v){os << "("; os << v.first << ", " << v.second << ")";return os;}
...
int main(void){
  ...
  while(++tcase, t--){
    ...
    string s[8];
    int f = 0;
    for(int i = 0; i < 8 && !f; ++i) cin >> s[i];
    for(int i = 0; i < 8; ++i){
        for(int j = 0; j < 8; ++j){
            int cnt = 0;
            if(s[i][j] == '#'){
                if(i >= 1 && j >= 1 && s[i - 1][j - 1] == '#') ++cnt;
                if(i >= 1 && j + 1 < 8 && s[i - 1][j + 1] == '#') ++cnt;
                if(i + 1 < 8 && j >= 1 && s[i + 1][j - 1] == '#') ++cnt;
                if(i + 1 < 8 && j + 1 < 8 && s[i + 1][j + 1] == '#') ++cnt;
            }
            if(cnt == 4){
                f = 1;
                cout << i + 1 << ' ' << j + 1 << ln;
