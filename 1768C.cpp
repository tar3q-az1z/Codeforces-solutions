...
int main(void){
	...
	while(++tcase, t--){
		...
		for(int i = 0; i < n; ++i){
			...
			m[a[i]].pb(i);
			if(sz(m[a[i]]) > 2) f = 1;
			if(sz(m[a[i]]) == 2) s.insert(a[i]);
		}
		if(f){
			msg(NO);
			continue;
		}
		for(int i = 1; i <= n; ++i) if(sz(m[i]) == 0) abs.push(i);
		vi p(n, 0), q(n, 0);
		for(int i = 0; i < n; ++i){
			if(sz(m[a[i]]) == 1){
				p[i] = q[i] = a[i];
		...
		if(sz(s) != 0){
			for(auto i: s){
				...
				int id1 = m[i][0];
				int id2 = m[i][1];
				p[id1] = i;p[id2] = abs.top();
				q[id1] = abs.top(); q[id2] = i;
				abs.pop();
			}
		}
		for(int i = 0; i < n; ++i){
			if(max(p[i], q[i]) != a[i]){f = 1; break;}
		}
		if(f) msg(NO);
		else{
			msg(YES);
			for(auto i: p) cout << i << ' '; cout << ln;
			for(auto i: q) cout << i << ' '; cout << ln;
		}
	}
	return 0;
}
