...
int main(void){
    ...
    while(++tcase, t--){
        ...
        for(auto &i: a) cin >> i;
        for(auto &i: b) cin >> i;
        map<int, int> fq, m1, m2;
        for(int i = 0; i < n; ++i) fq[a[i]] = i;
        for(int i = 0; i < n; ++i) {
            if(a[i] == b[i]) {
                m1[a[i]] = n;
                m2[a[i]] = n;
                ...
                if(fq[b[i]] > i) {
                    m2[b[i]] = fq[b[i]] - i;
                    m1[b[i]] = i + (n - 1 - fq[b[i]] + 1);
                }else {
                    m1[b[i]] = i - fq[b[i]];
                    m2[b[i]] = (n - 1 - i) + fq[b[i]] + 1;
                }
            }
        }
        map<int, int> cnt1, cnt2;
        int ans = 0;
        for(auto [k, v]: m1) cnt1[v]++;
        for(auto [k, v]: m2) cnt2[v]++;
        for(auto [k, v]: cnt1) ans = max(ans, v);
        for(auto [k, v]: cnt2) ans = max(ans, v);
        cout << ans << ln;
