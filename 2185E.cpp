...
int main(void){
    ...
    while(++tcase, t--){
        ...
        map<pair<int, int>, pair<int, int>> neighs;
        for(int i = 0; i < n; ++i) {
            auto it = upper_bound(begin(b), end(b), a[i]);
            if(it == end(b)) {
                before.insert({a[i] - b[m - 1], a[i]});
            }
            else if(it == begin(b)) {
                after.insert({b[0] - a[i], a[i]});
            }else {
                int d2 = *it, d1 = *(--it);
                pair<int, int> p1 = {a[i] - d1, a[i]}, p2 = {d2 - a[i], a[i]};
                //cout << d1 << ' ' << d2 << ln;
                before.insert(p1);
                after.insert(p2);
                neighs[p1] = p2;
        ...
        for(int i = 0; i < k; ++i) {
            ...
            if(now < 0) {
                while(!before.empty() && (*before.begin()).first <= -now) {
                    auto rec = *before.begin();
                    before.erase(before.begin());
                    after.erase(neighs[rec]);
                    --ans;
                }
            }else {
                while(!after.empty() && (*after.begin()).first <= now) {
                    auto rec = *after.begin();
                    after.erase(after.begin());
                    before.erase(neighs[rec]);
                    --ans;
                }
            }
            cout << max(ans, 0) << ' ';
