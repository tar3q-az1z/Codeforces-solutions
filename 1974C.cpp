...
int main(void){
	...
	while(++tcase, t--){
		...
		for(auto &i: a) cin >> i;
		// draw venn diagram with 3 set with common intersec., each set as a value of triplets
		map<tuple<int, int, int>, int> trio;
		long long int ans = 0ll;
		for(int i = 0; i < n - 2; ++i){
			long long int x = a[i], y = a[i + 1], z = a[i + 2];
			ans += 1ll * trio[make_tuple(x, y, 0)];
			ans += 1ll * trio[make_tuple(x, 0, z)];
			ans += 1ll * trio[make_tuple(0, y, z)];
			ans -= 3ll * trio[make_tuple(x, y, z)];
			++trio[make_tuple(x, y, 0)];
			++trio[make_tuple(x, 0, z)];
			++trio[make_tuple(0, y, z)];
			++trio[make_tuple(x, y, z)];
		}
		cout << ans << '\n';
	}
	return 0;
