...
int main(void){
	...
	while(++tcase, t--){
		...
		cin >> n0 >> n1 >> n2;
		string s;
		for(int i = 0; i <= n0 && n0; ++i) s.pb('0');
		if(n0 && n1){
			s = "1" + s;
			for(int i = 1; i < n1; ++i) s.pb(i % 2? '1': '0');
		}else if(n1){
			if(n1 % 2) for(int i = 0; i <= n1; ++i) s.pb((i + 1) % 2? '0': '1');
			else for(int i = 0; i <= n1; ++i) s.pb(i % 2? '0' : '1');
		}
		if(sz(s)){
			if(s[0] == '1') for(int i = 0; i < n2 && n2; ++i) s = "1" + s;
			else for(int i = 0; i < n2 && n2; ++i) s.pb('1');
		}else for(int i = 0; i <= n2 && n2; ++i) s.pb('1');
		cout << s << ln;
		//_timer_();
	}
