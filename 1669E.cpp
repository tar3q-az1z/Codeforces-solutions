...
int main(void){
	...
	while(++tcase, t--){
		...
		lli ans = 0ll;
		// first char fixed
		auto fun = [](lli p){return (p * (p - 1)) / 2;};
		for(char ch = 'a'; ch <= 'z'; ++ch){
			lli diff = m1[ch];
			ans += fun(diff);
			for(char r = 'a'; r <= 'z'; ++r) ans -= fun(m3[{ch, r}]);
		}
		// 2nd char fixed
		for(char ch = 'a'; ch <= 'z'; ++ch){
			lli diff = m2[ch];
			ans += fun(diff);
			for(char r = 'a'; r <= 'z'; ++r) ans -= fun(m3[{r, ch}]);
		}
		cout << ans << '\n';
	}
	return 0;
