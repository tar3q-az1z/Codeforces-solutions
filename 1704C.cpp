...
int main(void){
	...
	while(++tcase, t--){
		...
		long long int m, n; 
		cin >> n >> m; 
		vector<long long int> a(m), yet;
		for(auto &i: a) cin >> i;
		sort(begin(a), end(a));
		for(int i = 1; i < m; ++i){
			yet.pb(a[i] - a[i - 1] - 1);
		}
		yet.pb(n - a[m - 1] + a[0] - 1);
		sort(begin(yet), end(yet), greater<long long int>());
		long long int days = 0, safe = 0, now;
		for(auto &curr: yet){
			now = curr - 2 * days;
			if(now <= 0) break;
			safe += now - 1;
