...
int main(void){
	...
	while(++tcase, t--){
		...
		cin >> n >> k; 
		vii a(n);
		lli sum = 0ll, p_sum = 0ll, t_sum = 0ll; 
		for(lli i = 0; i < n; ++i){
			cin >> a[i];
			t_sum += a[i];
		}
		lli mx_found = -inf.l, mx_end = 0ll;
		for(lli i = 0; i < n; ++i){
			mx_end += a[i];
		...
		p_sum = mx_found;
		// cout << "psum: " << p_sum << ln;
		if(p_sum <= 0) cout << getMod(t_sum, MOD.l) << ln;
		else{
			lli ans = 0ll;
			for(lli i = 0; i < k; ++i){
				ans += getMod(getMod(p_sum, MOD.l) * binpow(2, i, MOD.l), MOD.l);
			}
			cout << getMod(ans + t_sum, MOD.l) << ln;
		}
	}
	return 0;
}
