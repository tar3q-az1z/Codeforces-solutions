...
int main(void){
    ...
    while(++tcase, t--){
        ...
        for(int i = 0; i < n; ++i) {
            ...
            if(a[i].first == a[i].second && m[{a[i].first, a[i].first}] == 1) s.pb(a[i].first);
        }
        sort(All(s));
        for(int i = 0; i < n; ++i) {
            if(a[i].first == a[i].second) {
                if(m[{a[i].first, a[i].first}] == 1) cout << 1;
                else cout << 0;
            }
            else {
                int r = distance(begin(s), upper_bound(begin(s), end(s), a[i].second));
                int l = distance(begin(s), lower_bound(begin(s), end(s), a[i].first));
                int dx = r - l;
