...
int main(void){
	...
	while(++tcase, t--){
		...
		scanv(a);
		scanv(b);
		int maxa = maxE(a), maxb = maxE(b);
		if(a[n - 1] != max(maxa, maxb) && b[n - 1] != max(maxa, maxb)) cout << "No" << ln;
		else{
			if(a[n - 1] == max(maxa, maxb)){
				for(int i = 0; i < n - 1; ++i){
					if(b[i] > b[n - 1]) swap(a[i], b[i]);
				}
			}else if(b[n - 1] == max(maxa, maxb)){
				for(int i = 0; i < n - 1; ++i){
					if(a[i] > a[n - 1]) swap(a[i], b[i]);
				}
			}
			maxa = maxE(a), maxb = maxE(b);
			if(maxa == a[n - 1] && maxb == b[n - 1]) cout << "Yes" << ln;
			else cout << "No" << ln;
		}
	}
	return 0;
