...
int main(void){
	...
	while(++tcase, t--){
		...
		vi a(n, 0);
		for(int i = 0; i < n; ++i){
			int f = [](int a, int b){if(a < b) return 1; else if(a % b) return a / b + 1; else return a / b;}(i + 1, k);
			// bug(f);
			int cnt = 0;
			for(int j = 0; j <= i; ++j) if(a[j] == 1) ++cnt;
			if(cnt < f) a[i] = 1;
		// printv(a);
			cnt = 0;
			for(int j = 0; j < i + 1; ++j) if(a[n - 1 - j] == 1) ++cnt; 
			if(cnt < f) a[n - i - 1] = 1;
		//printv(a);
		}
		int cnt = 0;
