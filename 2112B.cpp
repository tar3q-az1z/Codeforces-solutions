...
int main(void){
    ...
    while(++tcase, t--){
        ...
        for(int i = 0; i < n; ++i) {
            for(int k = i - 1; k >= 0; --k) {
                if(a[k] == a[i]) ans = min(ans, i - k - 1);
                if(a[k] + 1 == a[i]) ans = min(ans, i - k - 1);
                if(a[k] - 1 == a[i]) ans = min(ans, i - k - 1);
                if(k - 1 >= 0 && a[i] >= min(a[k], a[k - 1]) && a[i] <= max(a[k], a[k - 1])) ans = min(ans, i - k);
            }
            for(int k = i + 1; k < n; ++k) {
                if(a[k] == a[i]) ans = min(ans, k - i - 1);
                if(a[k] + 1 == a[i]) ans = min(ans, k - i - 1);
                if(a[k] - 1 == a[i]) ans = min(ans, k - i - 1);
                if(k + 1 < n && a[i] >= min(a[k], a[k + 1]) && a[i] <= max(a[k], a[k + 1])) ans = min(ans, k - i);
            }
        }
        if(ans < inf.i) cout << ans << ln;
        else cout << -1 << ln;
