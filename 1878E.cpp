...
void _period_(){
	...
	double delta = chrono::duration_cast<chrono::milliseconds>(end_time - start_time).count();
	FIXED(5);
	cout << "time : "<< delta <<" ms\n";
}
...
i64 NcR(i64 n, i64 r){i64 x = 1ll, y = 1ll;if(n - r < r) r = n - r; while(r){x *= n;y *= r;
i64 cm = gcd(x, y);x /= cm;y /= cm;--n;--r;}return x;}
...
template<typename T>
ostream& operator<< (ostream& out, const vector<T>& v){out << "[";size_t last = v.size() - 1;
for(size_t i = 0; i < v.size(); ++i){out << v[i];if (i != last)out << ", ";}out << "]";return out;}

// for set & set of ....
template <typename T> 
ostream& operator<<(ostream& os, const set<T>& v){os << "{";for(auto it : v){os << it;if(it != *v.rbegin()) 
os << ", ";}os << "}";return os;}

// for map, map of vector & .....
template <typename T, typename S> 
ostream& operator<<(ostream& os, const map<T, S>& v){for(auto it : v)  
os << it.first << " : " << it.second << "\n";return os;}

// for pair & pair of .......
template <typename T, typename S> 
ostream& operator<<(ostream& os, const pair<T, S>& v){os << "("; 
os << v.first << ", " << v.second << ")";return os;}

// array<T, n> a = {}

const i64 N = 2e5 + 5; // max limit of array
i64 n; // array size
vector<i64> Tree(4 * N); // seg tree

void build(){ // build the tree; leaves are stored in continuous nodes with indices starting with n
	for(i64 i = n - 1; i > 0; --i) Tree[i] = Tree[i << 1] & Tree[i << 1 | 1]; //eq. t[2i] + t[2i+1]

	// for(i64 i = n - 1; i > 0; --i) Tree[i] = min(Tree[i << 1], Tree[i << 1 | 1]);
	// for(i64 i = n - 1; i > 0; --i) Tree[i] = Tree[i << 1] & Tree[i << 1 | 1]; // for &
	// same goes for max, xor, gcd, &, |, ........
}

void update(i64 p, i64 val){ // set val at positions p(0 based)
	for(Tree[p += n] = val; p > 1; p >>= 1) Tree[p >> 1] = Tree[p] + Tree[p ^ 1]; // p^1 gives sibling of p
}

i64 query(i64 l, i64 r){ // oper. on [l, r] -< l,r are 1 based idx
	i64 res = (1ll << 62) - 1; // for &, set res = (1ll << 63) - 1; // all are ones
	for(l += n - 1, r += n; l < r; l >>= 1, r >>= 1){
		if(l & 1) res &= Tree[l++];
		if(r & 1) res &= Tree[--r];
		// draw a perfect binary tree to visualize
	}
	return res;
	// for min query.....
	// res = inf.l;
	// .....
	// if.... res = min(res, Tree[l++])
	// if.... res = min(res, Tree[--r])
	// same goes for ^, &, |, max ......
}
...
// in main:
// cin >> n;
// for(i64 i = 0; i < n; ++i) cin >> Tree[n + i];
// build();
