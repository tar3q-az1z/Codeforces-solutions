// @Author: Mars_Coder
// @date: 29-Jul-23 9:33:48 PM +06
// https://codeforces.com/problemset/problem/1855/C1
...
int main(void){
	...
	while(++tcase, t--){
		...
		if(mx <= 0){
			for(int i = n - 2; i >= 0; --i) {a[i] += a[i + 1]; ans.pb({i + 1, i + 2});}
			cout << sz(ans) << ln;
			//printv(a);
			for(auto j : ans) cout << Fi(j) << ' ' << Sc(j) << ln;
			continue;
		}
		//bug(a[mxd]);
		while(a[mxd] <= 20){
