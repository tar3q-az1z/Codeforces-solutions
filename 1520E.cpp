...
int main(void){
    ...
    while(++tcase, t--){
        ...
        for(i64 i = 0; i < n; ++i) {
            if(i) {
                if(s[i] == '*') pf[i] = pf[i - 1], ++curr;
                else pf[i] = pf[i - 1] + curr;
            }else if(s[i] == '*') ++curr;
        }
        curr = 0;
        for(i64 i = n - 1; i >= 0; --i) {
            if(i + 1 < n) {
                if(s[i] == '*') sf[i] = sf[i + 1], ++curr;
                else sf[i] = sf[i + 1] + curr;
            }else if(s[i] == '*') ++curr;
        }
        i64 ans = inf.l;
        for(i64 i = 0; i < n; ++i) {
            ans = min(ans, pf[i] + (i == n - 1 ? 0 : sf[i + 1]));
            ans = min(ans, sf[i] + (i == 0 ? 0 : pf[i - 1]));
        }
        cout << ans << ln;
