...
#define dbug(args...)     {string s = #args;replace(all(s), ',', ' '); stringstream s2; s2 << s; vss ss;\
...
template<typename T, typename... param> void debug(vss ss, T a, param... args){
...
int main(void){
	...
	while(++tcase, t--){
		...
		//a[1][n - 1] = ans.front();
		for(int i = 0; i < 2; ++i){
			for(int j = 0; j < n; ++j) cout << a[i][j] << ' '; cout << ln;
		}
	}
	return 0;
}
