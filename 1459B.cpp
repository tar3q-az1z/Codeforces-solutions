...
int main(void){
    ...
    while(++tcase, t--){
        int n;
        cin >> n;
        if(n % 2 == 0) cout << (n / 2 + 1) * (n / 2 + 1) << ln; // for n(even) case: 4, 9, 16
        else {
            int x = (n - 1) / 2 + 1; // n to be x th odd num
            cout << 2 * x * (x + 1) << ln; // for n(odd) case: 4, 12, 24
        }
