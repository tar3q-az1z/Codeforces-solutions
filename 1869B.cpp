...
int main(void){
	...
	while(++tcase, t--){
		...
		lli ans1 = 1e16, ans2 = 1e16;
		for(auto i = 0; i < k; ++i) ans1 = min(ans1, llabs(x[a - 1] - x[i]) + llabs(y[a - 1] - y[i]));
		for(auto i = 0; i < k; ++i) ans2 = min(ans2, llabs(x[b - 1] - x[i]) + llabs(y[b - 1] - y[i]));
