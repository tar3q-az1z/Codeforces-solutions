...
int main(void){
	...
	while(++tcase, t--){
		...
		string s;
		cin >> s;
		long long int n = sz(s);
		/* solve these cases, then try to .... 
			111111
			000000
			110000
			000111
			110001
			110111
			110101 -> this one alone covers 90% of cases


			hint: arrange them in cicle and count max. consecutive ones, and ......
		*/
		if(count(begin(s), end(s), '1') == n) cout << n * n << '\n'; 
		else{
			long long int cnt1(0), curr1(0);
			for(auto i: s){
				if(i == '1') ++curr1;
				else{
					cnt1 = max(cnt1, curr1);
			...
			while(i < n && s[i] == '1') ++i;
			while(j >= 0 && s[j] == '1') --j;
			long long int next = i + (n - j - 1);
			curr1 = 0;
			for(auto x = i; x <= j; ++x) if(s[x] == '1') ++curr1;
			if(curr1 == 0) cnt1 = count(begin(s), end(s), '1');
			long long int ans = 0ll;
			cnt1 = max(cnt1, next);
			for(i = 0; i < cnt1; ++i) ans = max(ans, (cnt1 - i) * (i + 1));
			cout << ans << '\n';
		}
	}
	return 0;
