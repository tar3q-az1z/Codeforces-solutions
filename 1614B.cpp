...
int main(void){
	...
	while(++tcase, t--){
		...
		for(int i = n - 1; i >= 0; --i){
			if(cnt > 0){
				ans += llabs(cnt) * 2ll * a[i].first;
				a[i].first = cnt;
				cnt = -cnt;
			}else{
				ans += abs(cnt) * 2ll * a[i].first;
				a[i].first = cnt;
				cnt = abs(cnt) + 1;
			}
		}
		sort(All(a), [](auto i, auto j){return i.second < j.second;});
		cout << ans << ln;
		cout << "0 ";
		for(int i = 0; i < n; ++i) cout << a[i].first << ' '; cout << ln;
	}
	return 0;
}
