	...
	while(++tcase, t--){
		...
		for(int i = 0; i < n / 2; ++i){
			...
			for(int j = i; j <= k; ++j){
				int cnt = 0;
				int p = n - 1 - j;
				if(a[i][j] == '1') ++cnt;  // from top left corner to right
				if(a[j][k] == '1') ++cnt;  // from top right corner to bottom
				if(a[k][p] == '1') ++cnt;  // from bottom right corner to left
				if(a[p][i] == '1') ++cnt;  // from bottom left corner to top
				if(cnt <= 2){
					a[i][j] = a[j][k] = a[k][p] = a[p][i] = '0';
					ans += cnt;
				}
				else{
					ans += 4 - cnt;
					a[i][j] = a[j][k] = a[k][p] = a[p][i] = '1';
				}
				// dbug(i, j, k, p, cnt, ans);
			}
		}
		cout << ans << ln;
		//_timer_();
