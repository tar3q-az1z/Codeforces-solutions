...
int main(void){
    ...
    while(++tcase, t--){
        ...
        //cout << ans << ln;
        for(int i = l - 1, k = r + 1; i >= 0 && k < 2 * n && sz(ans); --i, ++k) {
            if(a[i] == a[k]) ans.insert(a[i]);
            else break;
        }
        int res = sz(ans);
        ...
        ans2.insert(0);
        ans3.insert(0);
        for(int i = l - 1, k = l + 1; i >= 0 && k < 2 * n; --i, ++k) {
            if(a[i] == a[k]) {
                ans2.insert(a[i]);
            }
            else break;
        }
        for(int i = r - 1, k = r + 1; i >= 0 && k < 2 * n; --i, ++k) {
            if(a[i] == a[k]) {
                ans3.insert(a[i]);
            }else break;
        }
        int res2 = sz(ans2), res3 = sz(ans3);
        ...
        for(int i = 0; i < sz(ans3); ++i) {
            if(ans3.count(i) == 0) {
                res3 = i;
                break;
            }
        }
        res = max(res, max(res2, res3));
        cout << res << ln;
