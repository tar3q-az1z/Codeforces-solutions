	...
	while(++tcase, t--){
		...
		for(int i = 0; i < sz(s); ++i){
			if(s[i] == 'b' && !lasts.empty()){
				s[lasts[sz(lasts) - 1]] = '0';
				lasts.pop_back();
				s[i] = '0';
			}else if(s[i] == 'B' && !lastu.empty()){
				s[lastu[sz(lastu) - 1]] = '0';
				lastu.pop_back();
				s[i] = '0';
			}else if(s[i] >= 97 && s[i] != 'b') lasts.pb(i);
			else if(s[i] != 'B' && s[i] <= 90) lastu.pb(i);
			else s[i] = '0';
		}
		for(int i = 0; i < sz(s); ++i) if(s[i] != '0') cout << s[i];
		cout << ln;
	}
	return 0;
}
