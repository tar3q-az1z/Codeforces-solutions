...
int main(void){
	...
	while(++tcase, t--){
		...
		if(f) cout << "0.000000000000" << ln;
		else{
			lli qm = 0;
			for(auto i: s2) if(i == '?') ++qm;
			auto fact = [](lli x){lli ans = 1; for(lli i = 1; i <= x; ++i) ans *= i; return ans;};
			lli fav = fact(qm) / (fact(p) * fact(n));
			FIXED(12);
			cout << (fav * 1.00000) / (1 << qm) << ln;
		}
	}
	return 0;
}
