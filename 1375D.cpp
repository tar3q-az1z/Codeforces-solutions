...
class Mex{
	...
	public:
		Mex(vector<int> const& A) : A(A){
			for(int i = 0; i <= A.size(); ++i) missing.insert(i);
			for(int x: A){
				++freq[x];
				missing.erase(x);
			}
		...
		void update(int idx, int new_value){
			if(--freq[A[idx]] == 0) missing.insert(A[idx]);
			A[idx] = new_value;
			++freq[new_value];
			missing.erase(new_value);
		}
}; // doc: Mex mexc(vector); mexc.mex(), mexc.update(idx, val)
