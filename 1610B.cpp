...
int main(void){
	...
	while(++tcase, t--){
		...
		auto ck = [&](int x){
			vector<int> tm; 
			for(auto i: a) if(i != x) tm.pb(i);
			ok = ok || (tm == vector<int>(rbegin(tm), rend(tm)));
		};
		for(int i = 0; i < n; ++i){
			if(a[i] != a[n - 1 - i]){ // palindrome check
				ck(a[i]);
				ck(a[n - 1 - i]);
				break;
			}
		}
		ok = ok || (a == vector<int>(rbegin(a), rend(a)));
		cout << (ok ? "YES\n" : "NO\n");
	}
	return 0;
}
